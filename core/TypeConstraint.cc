...
bool TypeConstraint::solve(const GlobalState &gs) {
    ...
    for (auto &k : upperBounds) {
        ...
        if (upperBound) {
            cantSolve = !Types::isSubType(gs, findSolution(tv), upperBound, core::noOpErrorSectionCollector);
            if (cantSolve) {
                return false;
            }
        }
    ...
    for (auto &k : lowerBounds) {
        ...
        cantSolve = !Types::isSubType(gs, lowerBound, findSolution(tv), core::noOpErrorSectionCollector);
        if (cantSolve) {
            return false;
        }
    }
...
bool TypeConstraint::isAlreadyASubType(const GlobalState &gs, const TypePtr &t1, const TypePtr &t2) const {
    if (auto t1p = cast_type<TypeVar>(t1)) {
        if (!hasLowerBound(t1p->sym)) {
            return Types::isSubType(gs, Types::top(), t2, core::noOpErrorSectionCollector);
        }
        return Types::isSubType(gs, findLowerBound(t1p->sym), t2, core::noOpErrorSectionCollector);
    } else {
        auto t2p = cast_type<TypeVar>(t2);
        ENFORCE(t2p != nullptr);
        if (!hasUpperBound(t2p->sym)) {
            return Types::isSubType(gs, t1, Types::bottom(), core::noOpErrorSectionCollector);
        }
        return Types::isSubType(gs, t1, findUpperBound(t2p->sym), core::noOpErrorSectionCollector);
    }
}
