...
TypePtr Types::any(const GlobalState &gs, const TypePtr &t1, const TypePtr &t2) {
    ...
    SLOW_ENFORCE(Types::isSubType(gs, t1, ret, core::noOpErrorSectionCollector),
                 "\n{}\nis not a super type of\n{}\nwas lubbing with {}", ret.toString(gs), t1.toString(gs),
                 t2.toString(gs));
    SLOW_ENFORCE(Types::isSubType(gs, t2, ret, core::noOpErrorSectionCollector),
...
TypePtr glbDistributeAnd(const GlobalState &gs, const TypePtr &t1, const TypePtr &t2) {
    ...
    if (Types::isSubType(gs, n1, n2, core::noOpErrorSectionCollector)) {
        categoryCounterInc("glbDistributeAnd.outcome", "ZZn1");
        return n1;
    } else if (Types::isSubType(gs, n2, n1, core::noOpErrorSectionCollector)) {
...
TypePtr dropLubComponents(const GlobalState &gs, const TypePtr &t1, const TypePtr &t2) {
    if (auto *a1 = cast_type<AndType>(t1)) {
        ...
        auto subl = Types::isSubType(gs, a1a, t2, core::noOpErrorSectionCollector);
        auto subr = Types::isSubType(gs, a1b, t2, core::noOpErrorSectionCollector);
        if (subl || subr) {
            return Types::bottom();
        }
        ...
        auto subl = Types::isSubType(gs, o1->left, t2, core::noOpErrorSectionCollector);
        auto subr = Types::isSubType(gs, o1->right, t2, core::noOpErrorSectionCollector);
        if (subl && subr) {
            return Types::bottom();
        } else if (subl) {
            return o1->right;
...
TypePtr Types::lub(const GlobalState &gs, const TypePtr &t1, const TypePtr &t2) {
    ...
    if (auto *a1 = cast_type<AppliedType>(t1)) {
        ...
        if (a2 == nullptr) {
            if (isSubType(gs, t2, t1, core::noOpErrorSectionCollector)) {
                return t1;
            }
            if (isSubType(gs, t1, t2, core::noOpErrorSectionCollector)) {
                return t2;
            }
        ...
        for (SymbolRef idx : a2->klass.data(gs)->typeMembers()) {
            ...
            if (idxTypeMember.data(gs)->flags.isCovariant) {
                ...
                if (!Types::equiv(gs, a1->targs[i], a2->targs[j], core::noOpErrorSectionCollector)) {
    ...
    if (is_proxy_type(t1)) {
        ...
        if (is_proxy_type(t2)) {
            ...
            typecase(
                ...
                [&](const MetaType &m1) {
                    if (auto *m2 = cast_type<MetaType>(t2)) {
                        if (Types::equiv(gs, m1.wrapped, m2->wrapped, core::noOpErrorSectionCollector)) {
                            result = t1;
                            return;
                        }
                    }
            ...
            if (isSubType(gs, und, t2, core::noOpErrorSectionCollector)) {
                return t2;
        ...
        if (isSubType(gs, und, t1, core::noOpErrorSectionCollector)) {
            return t1;
        } else if (allowProxyInLub) {
            return OrType::make_shared(t1, t2);
        } else {
...
TypePtr Types::all(const GlobalState &gs, const TypePtr &t1, const TypePtr &t2) {
    ...
    SLOW_ENFORCE(Types::isSubType(gs, ret, t1, core::noOpErrorSectionCollector),
                 "\n{}\nis not a subtype of\n{}\nwas glbbing with\n{}", ret.toString(gs), t1.toString(gs),
                 t2.toString(gs));

    SLOW_ENFORCE(Types::isSubType(gs, ret, t2, core::noOpErrorSectionCollector),
...
TypePtr Types::glb(const GlobalState &gs, const TypePtr &t1, const TypePtr &t2) {
    ...
    if (is_proxy_type(t1)) {
        if (is_proxy_type(t2)) {
            ...
            // only 1st is proxy
            if (Types::isSubType(gs, t1, t2, core::noOpErrorSectionCollector)) {
                return t1;
            } else {
                return Types::bottom();
            }
        ...
        // only 1st is proxy
        if (Types::isSubType(gs, t2, t1, core::noOpErrorSectionCollector)) {
            return t2;
        } else {
            return Types::bottom();
        }
    ...
    if (auto *o2 = cast_type<OrType>(t2)) { // 3, 6
        bool collapseInLeft = Types::isAsSpecificAs(gs, t1, t2, core::noOpErrorSectionCollector);
        if (collapseInLeft) {
            categoryCounterInc("glb", "Zor");
            return t1;
        }

        bool collapseInRight = Types::isAsSpecificAs(gs, t2, t1, core::noOpErrorSectionCollector);
        if (collapseInRight) {
        ...
        if (isa_type<ClassType>(t1) || isa_type<AppliedType>(t1)) {
            ...
            if (Types::isAsSpecificAs(gs, lft, o2->right, core::noOpErrorSectionCollector) && !lft.isBottom()) {
                categoryCounterInc("glb", "ZZZorClass");
                return lft;
            }
            auto rght = Types::all(gs, t1, o2->right);
            if (Types::isAsSpecificAs(gs, rght, o2->left, core::noOpErrorSectionCollector) && !rght.isBottom()) {
        ...
        for (auto idx : a1->klass.data(gs)->typeMembers()) {
            ...
            if (i >= a2->klass.data(gs)->typeMembers().size()) { // a1 has more tparams, this is fine, it's a child
                ...
                if (a2TypeMember.data(gs)->flags.isCovariant) {
                    ...
                    if (!Types::equiv(gs, a1->targs[j], a2->targs[i], core::noOpErrorSectionCollector)) {
                        return AndType::make_shared(t1, t2);
                    }
                    const auto &lambdaParam = cast_type<LambdaParam>(idx.data(gs)->resultType);
                    if (a1->targs[j].isUntyped() &&
                        Types::isSubType(gs, lambdaParam->lowerBound, a2->targs[i], core::noOpErrorSectionCollector) &&
                        Types::isSubType(gs, a2->targs[i], lambdaParam->upperBound, core::noOpErrorSectionCollector)) {
...
bool isSubTypeUnderConstraintSingle(const GlobalState &gs, TypeConstraint &constr, UntypedMode mode, const TypePtr &t1,
    ...
    if (isa_type<AppliedType>(t2)) {
        if (is_proxy_type(t1)) {
            return Types::isSubTypeUnderConstraint(gs, constr, t1.underlying(gs), t2, mode, errorSectionCollector);
        }
        return false;
    }
        ...
        if (is_proxy_type(t2)) {
            ...
            typecase(
                ...
                [&](const MetaType &m1) {
                    ...
                    result = Types::equiv(gs, m1.wrapped, m2->wrapped, errorSectionCollector);
                });
            return result;
            // both are proxy
        } else {
            // only 1st is proxy
            TypePtr und = t1.underlying(gs);
            return isSubTypeUnderConstraintSingle(gs, constr, mode, und, t2, errorSectionCollector);
        }
    } else if (is_proxy_type(t2)) {
...
template <class T>
bool Types::isAsSpecificAs(const GlobalState &gs, const TypePtr &t1, const TypePtr &t2, T &errorSectionCollector) {
    return isSubTypeUnderConstraint(gs, TypeConstraint::EmptyFrozenConstraint, t1, t2, UntypedMode::AlwaysIncompatible,
                                    errorSectionCollector);
}

template <class T>
bool Types::isSubType(const GlobalState &gs, const TypePtr &t1, const TypePtr &t2, T &errorSectionCollector) {
...
bool Types::isSubTypeUnderConstraint(const GlobalState &gs, TypeConstraint &constr, const TypePtr &t1,
    ...
    if (auto *o1 = cast_type<OrType>(t1)) { // 7, 8, 9
        return Types::isSubTypeUnderConstraint(gs, constr, o1->left, t2, mode, errorSectionCollector) &&
               Types::isSubTypeUnderConstraint(gs, constr, o1->right, t2, mode, errorSectionCollector);
    }

    if (auto *a2 = cast_type<AndType>(t2)) { // 2, 5
        return Types::isSubTypeUnderConstraint(gs, constr, t1, a2->left, mode, errorSectionCollector) &&
               Types::isSubTypeUnderConstraint(gs, constr, t1, a2->right, mode, errorSectionCollector);
    }
    ...
    if (o2 != nullptr) { // 3
        ...
        auto leftIsSubType = Types::isSubTypeUnderConstraint(gs, constr, t1, o2->left, mode, errorSectionCollector);
        ...
        if (leftIsSubType && !stillNeedToCheckRight) {
            ...
            return true;
        } else if (Types::isSubTypeUnderConstraint(gs, constr, t1, o2->right, mode, errorSectionCollector)) {
            return true;
        } else if (a1 == nullptr) {
    ...
    if (a1 != nullptr) { // 4
        ...
        auto leftIsSubType = Types::isSubTypeUnderConstraint(gs, constr, a1->left, t2, mode, errorSectionCollector);
        auto stillNeedToCheckRight = t2.isUntyped() && a1->left.isFullyDefined() && !a1->right.isFullyDefined();
        if (leftIsSubType && !stillNeedToCheckRight) {
            // Short circuit to save time
            return true;
        } else {
            return Types::isSubTypeUnderConstraint(gs, constr, a1->right, t2, mode, errorSectionCollector);
        }
    }

    return isSubTypeUnderConstraintSingle(gs, constr, mode, t1, t2, errorSectionCollector); // 1
}

template <class T>
bool Types::equiv(const GlobalState &gs, const TypePtr &t1, const TypePtr &t2, T &errorSectionCollector) {
    return isSubType(gs, t1, t2, errorSectionCollector) && isSubType(gs, t2, t1, errorSectionCollector);
}

template <class T>
bool Types::equivUnderConstraint(const GlobalState &gs, TypeConstraint &constr, const TypePtr &t1, const TypePtr &t2,
                                 T &errorSectionCollector) {
    auto mode = UntypedMode::AlwaysCompatible;
    return isSubTypeUnderConstraint(gs, constr, t1, t2, mode, errorSectionCollector) &&
           isSubTypeUnderConstraint(gs, constr, t2, t1, mode, errorSectionCollector);
}

template <class T>
bool Types::equivNoUntyped(const GlobalState &gs, const TypePtr &t1, const TypePtr &t2, T &errorSectionCollector) {
    return isAsSpecificAs(gs, t1, t2, errorSectionCollector) && isAsSpecificAs(gs, t2, t1, errorSectionCollector);
}
...
template bool isSubTypeUnderConstraintSingle(const GlobalState &gs, TypeConstraint &constr, UntypedMode mode,
                                             ...
                                             core::ErrorSectionCollector &errorSectionCollector);
template bool isSubTypeUnderConstraintSingle(const GlobalState &gs, TypeConstraint &constr, UntypedMode mode,
                                             const TypePtr &t1, const TypePtr &t2,
                                             core::NoOpErrorSectionCollector &errorSectionCollector);
...
template bool Types::equiv(const GlobalState &gs, const TypePtr &t1, const TypePtr &t2,
                           NoOpErrorSectionCollector &errorSectionCollector);

template bool Types::equivUnderConstraint(const GlobalState &gs, TypeConstraint &constr, const TypePtr &t1,
                                          const TypePtr &t2, core::ErrorSectionCollector &errorSectionCollector);
template bool Types::equivUnderConstraint(const GlobalState &gs, TypeConstraint &constr, const TypePtr &t1,
                                          const TypePtr &t2, core::NoOpErrorSectionCollector &errorSectionCollector);
