...
void Environment::updateKnowledge(core::Context ctx, cfg::LocalRef local, core::Loc loc, const cfg::Send *send,
    ...
    if (send->fun == core::Names::blank_p()) {
        ...
        if (!core::Types::equiv(ctx, knowledgeTypeWithoutFalsy, originalType, core::noOpErrorSectionCollector)) {
            auto &whoKnows = getKnowledge(local);
    ...
    try {
        ...
        typecase(
            ...
            [&](cfg::BlockReturn &i) {
                ...
                if (core::Types::isSubType(ctx, core::Types::void_(), expectedType, core::noOpErrorSectionCollector)) {
                ...
                if (i.link->result->main.constr) {
                    isSubtype = core::Types::isSubTypeUnderConstraint(
                        ...
                        core::UntypedMode::AlwaysCompatible, errorSectionCollector);
                } else {
                    isSubtype = core::Types::isSubType(ctx, typeAndOrigin.type, expectedType, errorSectionCollector);
                }
                if (!isSubtype) {
                    if (auto e = ctx.beginError(bind.loc, core::errors::Infer::ReturnTypeMismatch)) {
            ...
            [&](cfg::Cast &c) {
                ...
                if (c.cast != core::Names::cast()) {
                    ...
                    } else if (!core::Types::isSubType(ctx, ty.type, castType, errorSectionCollector)) {
                        ...
                        if (castType.isUntyped()) {
                            if (auto e = ctx.beginError(bind.loc, core::errors::Infer::InvalidCast)) {
                                ...
                                if (argLoc.exists()) {
                                   ...
                                   core::Types::isSubType(ctx, ty.type, castType, core::noOpErrorSectionCollector)) {
                            if (auto e = ctx.beginError(bind.loc, core::errors::Infer::InvalidCast)) {
        ...
        if (!noLoopChecking && loopCount != bindMinLoops) {
            ...
            bool asGoodAs = core::Types::isSubType(ctx, core::Types::dropLiteral(ctx, tp.type),
                                                   core::Types::dropLiteral(ctx, cur.type), errorSectionCollector);
